 ...
 *
 * Two-column layout: left = pitch (eyebrow, heading, description, checklist,
 * CTA), right = 2×2 card grid (3 gauge metric cards + Reality Check™ card).
 *
 * Layout, card structure, SVG gauges, animation hooks, and CSS class names
 * are all unchanged from the previous version — only the content sourced
 * from inc/assessment-engine.php has been updated (outcome-focused
 * messaging throughout). See that file for content decisions and rationale.
 *
 * Progressive enhancement: stroke-dashoffset values are pre-computed
 * server-side so gauges show correct final values with JS disabled.
 * assets/js/assessment-engine.js animates from 0 when the section enters
 * the viewport.
 */
    ...
    <div class="du-assessment__inner du-container">
        ...
        <div class="du-assessment__content">
            ...
            <p class="du-assessment__eyebrow du-text-label-caps">
                <?php esc_html_e( 'Financial Assessment', 'downside-up' ); ?>
            </p>
