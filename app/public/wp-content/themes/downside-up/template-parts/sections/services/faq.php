...
/**
 * Services page FAQ.
 * Loaded via: get_template_part('template-parts/sections/services/faq');
 *
 * Reuses the site's FAQ accordion component wholesale — same .du-faq
 * classes and behaviour (assets/js/about-page/faq-accordion.js, already
 * enqueued site-wide) as
 * template-parts/sections/about-page/faq-accordion.php. No new CSS or JS.
 */

$du_faq = downside_up_get_services_faq_data();
$du_faq_items = $du_faq['items'];
$du_faq_support = $du_faq['support_note'];
?>

<section class="du-faq" aria-labelledby="du-services-faq-heading">
    <div class="du-container">
        ...
        <div class="du-faq__header">
            ...
            <h2 id="du-services-faq-heading" class="du-faq__heading du-text-headline-xl">
        ...
        <?php if (!empty($du_faq_items)) : ?>
            <div class="du-faq__accordion">
                <?php foreach ($du_faq_items as $index => $item) :
                    $item_id = 'services-faq-item-' . ($index + 1);
                    $trigger_id = 'services-faq-trigger-' . ($index + 1);
                    $panel_id = 'services-faq-panel-' . ($index + 1);
                ?>
